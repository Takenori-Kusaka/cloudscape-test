...
const editableColumns = {
  state: {
    ...
    editConfig: {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      editingCell: (item: any, { setValue, currentValue }: { setValue: any, currentValue: string }) => {
        const options = [
  ...
  domainName: {
    ...
    editConfig: {
      ...
      editIconAriaLabel: 'editable',
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      validation(item: Map<string, any>, value: string) {
  ...
  sslCertificate: {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
export const EDITABLE_COLUMN_DEFINITIONS = COLUMN_DEFINITIONS.map((column: any) => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  if ((editableColumns as any)[column.id]) {
    return {
      ...column,
      minWidth: Math.max(column.minWidth || 0, 176),
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...(editableColumns as any)[column.id],
    };
  }
  return column;
});
